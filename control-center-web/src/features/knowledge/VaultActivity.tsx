...
type Day = { activityReport?: {markdown:string}; activityUnavailable?: boolean; markdown: string; personalDiary: { markdown?: string } | null; sources: unknown[] };
type Policy = { inbox: string; personalDiary: string; remoteProcessing: boolean; jevEnabled: boolean };
...
export function VaultActivity({ vaultId, notes, view }: { vaultId: string; notes: Note[]; view: 'day' | 'review' | 'settings' }) {
  ...
  const day = useQuery({ queryKey: ['vault-day', vaultId, date, project, includeActivity], queryFn: () => call<Day>({ action: 'day', ...dayArgs }), enabled: view === 'day' });
  ...
  return <section className="vault-activity" aria-label={view === 'day' ? '日期页' : view === 'review' ? '笔记修订' : '笔记设置'}>
    ...
    {view === 'day' ? <>
      ...
      </section><section><h3>已获准的项目活动</h3>{day.data?.activityReport?<ReactMarkdown components={{img:({alt})=><span>{alt}</span>}}>{day.data.activityReport.markdown}</ReactMarkdown>:<p>开启后读取现有 Memory 准入链中的来源，不开启被动采集、不补写未取得的对话。</p>}</section><section><h3>我的日记</h3>{day.data?.personalDiary?.markdown ? <ReactMarkdown components={{img:({alt})=><span>附件：{alt || "图片"}（请在原笔记查看）</span>}}>{day.data.personalDiary.markdown}</ReactMarkdown> : <p>未授权日记或当天文件不存在。可在设置指定日期文件，不参与远程整理。</p>}</section>
    </> : null}
    {view === 'review' ? <>
      ...
      </section><section><h3>采纳为项目事项</h3><p>这是独立于改文的动作。只采纳你明确选定的原文陈述，不采纳整篇笔记。</p><label>精确原文陈述<textarea rows={3} value={statement} onChange={e=>setStatement(e.target.value)}/></label><button disabled={busy||!target||!project.trim()||!statement.trim()} onClick={()=>void run(async()=>{const n=notes.find(n=>n.id===target);const r=await call<{notice:string}>({action:'adopt_memory',noteId:target,baseRevision:n?.revision??'',statement,project,confirm:true});setNotice(r.notice);await refresh();})}>明确采纳到本项目 Memory</button><ul>{adoptions.data?.items.map(item=><li key={item.id}>{item.statement} · {item.state==='adopted'?'已采纳':item.state==='needs_review'?'依据已变化，已停用，等待复核':'处理中'}</li>)}</ul></section><section><h3>下次继续用</h3><p>重新读取目标笔记当前版本，生成本次讨论参考。不会自动发送或执行。</p><button disabled={busy || !target} onClick={() => void run(async () => { const r = await call<{ markdown: string; notice: string }>({ action: 'context', noteIds: [target] }); setPack(r.markdown); setNotice(r.notice); })}>预览本次参考</button>{pack ? <><textarea aria-label="本次讨论参考" readOnly rows={8} value={pack} /><button onClick={() => void run(async () => { const r = await call<{ markdown: string }>({ action: 'context', noteIds: [target] }); if (r.markdown !== pack) { setPack(r.markdown); throw new Error('内容已更新，请核对新的预览后再复制。'); } await navigator.clipboard.writeText(pack); setNotice('已复制，请确认目标应用后自行粘贴。'); })}>复制参考</button></> : null}</section>
    </> : null}
    {view === 'settings' ? <>
      ...
      <section><h4>个人日记</h4><p>仅日期页本地阅读。当前：{policy.data?.policy.personalDiary || '未授权'}</p><form onSubmit={e => { e.preventDefault(); void run(() => configure({ personalDiary })); }}><label>日期文件规则<input value={personalDiary} onChange={e => setPersonalDiary(e.target.value)} placeholder="日记/{date}.md" /></label><button disabled={busy}>保存日记读取范围</button></form></section>
