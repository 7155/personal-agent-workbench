...
import { ArrowUpRight, CheckCircle2, ChevronRight, CircleAlert, Clock3, FileText, FlaskConical, FolderOpen, GitBranch, Link2, LoaderCircle, MessageSquare, PackageCheck } from 'lucide-react';
...
export function LabProjectSpace({ project, artifactId, artifactContent, onSelectArtifact, onOpenNode, onOpenGraph, onOpenRuns, onOpenApps, onOpenChat, onOpenFile, sourceContent, onOpenSource, onCloseSource, selectedNodeId, onSelectNode }: {
  ...
  return <section className="lab-space" aria-label="项目成果工作面">
    ...
    <div className="lab-space__body"><aside className="lab-space__objects" aria-label="项目对象">
      ...
      <nav aria-label="工作成果">{project.artifacts.map((artifact) => <button key={artifact.artifactId} aria-current={!selected && artifactId === artifact.artifactId ? 'page' : undefined} onClick={() => selectArtifact(artifact.artifactId)} title={artifact.title}><FileText size={14} /><span>{artifact.title}<small>v{artifact.revision} · {artifact.view === 'html' ? '交互成果' : artifact.view === 'table' ? '对照表' : artifact.view === 'markdown' ? '文档' : '结果记录'}</small></span>{!selected && artifactId === artifact.artifactId ? <Link2 className="lab-space__objects-linked" size={13} aria-label="当前已关联到 Agent" /> : null}</button>)}</nav>
      ...
      <section className="lab-space-dimensions" aria-label="优化方向的实际测试记录"><header><h3>优化方向</h3><div className="lab-space-dimensions__actions">{(selected || artifactId) ? <span className="lab-space-dimensions__sync"><Link2 size={13} />已同步到 Agent</span> : null}<Button size="small" variant="quiet" onClick={onOpenChat}><MessageSquare size={14} />与 Agent 调整方向</Button></div></header><div>{dimensions.map((dimension) => {
        ...
        return <button key={dimension} aria-label={`${dimension} ${running ? workflowNodeStatus(running) : completed.length ? completedLabel : records.length ? '对照未完成' : '未记录测试'}`} aria-pressed={target?.id === selection} disabled={!target} onClick={() => target && selectNode(target)} data-running={Boolean(running)} data-selected={target?.id === selection || undefined}><strong>{dimension}</strong><span>{running ? workflowNodeStatus(running) : completed.length ? completedLabel : records.length ? '对照未完成' : '未记录测试'}</span></button>;
      })}</div></section>
