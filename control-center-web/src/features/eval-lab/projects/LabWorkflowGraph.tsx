...
export function WorkflowMetricComparison({ metric }: { metric: LabWorkflowMetric }) {
  ...
  if (metric.value !== undefined) return <div className="lab-flow-metric"><div className="lab-flow-metric__title"><strong>{metric.label}</strong><span>本次结果{metric.sampleCount !== undefined ? ` · ${metric.sampleCount} 题` : ''}</span></div><b>{display(metric.value)}</b><small className="lab-flow-metric__note">本次单独测量，尚无可比基线</small></div>;
