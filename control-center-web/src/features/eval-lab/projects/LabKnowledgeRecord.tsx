...
export function LabKnowledgeRecord({ projectId, jobId, onClose, onOpenKnowledge }: { projectId: string; jobId: string; onClose?: () => void; onOpenKnowledge?: () => void }) {
  ...
  return <aside className="lab-space-source lab-knowledge-record" aria-label="原知识库任务记录"><header><h3>原知识库任务 · {state}</h3><div>{onOpenKnowledge ? <Button size="small" onClick={onOpenKnowledge}>在知识库中查看</Button> : null}{onClose ? <Button size="small" onClick={onClose}>收起原任务</Button> : null}</div></header><p>原任务：{jobId}</p>
    {query.isPending ? <p role="status">正在读取原任务…</p> : query.isError ? <><p role="alert">{projectError(query.error)}</p><Button onClick={() => void query.refetch()}>重新读取原任务</Button></> : <>
      ...
      {typeof profile.mode === 'string' ? <p>本次检索：{profile.mode} · Top-K {metric(profile.topK)} · 上下文预算 {metric(profile.contextChars)}</p> : null}
      {Object.keys(metrics).length ? <section aria-label="本次检索指标"><h4>本次实际指标</h4><p>这是原任务的单次结果，尚未指定配对基线。</p><dl><dt>MRR</dt><dd>{metric(metrics.mrr)}</dd>{['recallAtK', 'ndcgAtK'].map((name) => Object.entries(object(metrics[name])).map(([k, value]) => <div key={`${name}:${k}`}><dt>{name === 'recallAtK' ? 'Recall' : 'nDCG'}@{k}</dt><dd>{metric(value)}</dd></div>))}</dl></section> : null}
      {hits.length ? <section aria-label="本次实际检索来源"><h4>实际命中的来源</h4>{hits.map((hit, index) => <article key={String(hit.chunkId ?? index)}><h5>{String(hit.title || hit.sourceId || '来源')}</h5><p className="golden-preserve-text">{typeof hit.content === 'string' ? hit.content : '此记录未返回正文。'}</p><small>{String(hit.sourceId || '')} · {String(hit.chunkId || '')}</small>{typeof hit.uri === 'string' ? <p>{hit.uri}</p> : null}</article>)}</section> : null}
