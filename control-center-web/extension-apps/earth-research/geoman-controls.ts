...
type Events={select:(feature:EditableGeometry|null,mode?:SelectionMode|'select')=>void;change:(features:EditableGeometry[])=>Promise<void>|void;active:(active:boolean,kind?:DrawingKind,vertices?:number)=>void;saving?:(pending:boolean)=>void;error?:(message:string)=>void};
...
export function drawingControls(map:L.Map,initial:EditableGeometry[],events:Events) {
  ...
  function replace(features:EditableGeometry[]) {
    ...
    if(kind==='edit')group.eachLayer(layer=>(layer as DataLayer).pm?.enable(options()));
  }
  ...
  function checkpoint(){const state=all();if(JSON.stringify(state)!==JSON.stringify(undo.at(-1))){undo.push(structuredClone(state));redo=[];}}
  function disableModes(){pm.disableDraw();pm.disableGlobalCutMode();group.eachLayer(layer=>(layer as DataLayer).pm?.disable());}
  function end(){kind=undefined;before=undefined;undo=[];redo=[];events.active(false);}
  ...
  function cancel(){if(saving||disposed)return;disableModes();if(before)replace(before);cleanupCopies();end();}
  function start(next:DrawingKind){
    if(saving||disposed)return;
    if(kind)cancel();
    kind=next;before=structuredClone(all());undo=[structuredClone(before)];redo=[];
    events.active(true,next,0);
    if(next==='edit')group.eachLayer(layer=>(layer as DataLayer).pm?.enable(options()));
    else if(next==='cut'){const cutOptions={...options(),layersToCut:group.getLayers()};pm.enableGlobalCutMode(cutOptions);}
  ...
  return {add,start,save,cancel,finish(){if(!saving&&(kind==='polygon'||kind==='polyline')){const handler=(pm.Draw as any)[shapeNames[kind]];handler?._finishShape();}},
