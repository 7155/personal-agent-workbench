...
export function EarthMap({ run, onSelect, command, selection, workspaceKey, onActivity, onMapState, projectLayers = EMPTY_PROJECT_LAYERS, spatialSources = [], workspaceFiles = [], activeObjectLabel, onSaveLayer, onUpdateFeature, onCreateBundle, onExportLayer, onToggleLayer, onRemoveLayer, onConnectSource, onRefreshCatalog, onRefreshFiles, onOpenFile }: { run: EarthRun | null; onSelect: (feature: GeoJSON.Feature | null,mode?:SelectionMode) => void; command?: EarthViewCommand; selection: GeoJSON.Feature[]; workspaceKey: string; onActivity: () => void; onMapState?: (state: EarthMapState) => void; projectLayers?: ProjectLayer[]; spatialSources?: SpatialSourceSummary[]; workspaceFiles?: WorkspaceFileSummary[]; activeObjectLabel?: string; onSaveLayer?: (name: string, features: GeoJSON.Feature[]) => Promise<void> | void; onUpdateFeature?: (feature: GeoJSON.Feature) => Promise<void> | void; onCreateBundle?: () => Promise<void> | void; onExportLayer?: (format: 'shp' | 'gpkg', layer: ProjectLayer) => Promise<void> | void; onToggleLayer?: (layerId: string, visible: boolean) => Promise<void> | void; onRemoveLayer?: (layerId: string) => Promise<void> | void; onConnectSource?: (source: SpatialSourceDraft) => Promise<void> | void; onRefreshCatalog?: () => Promise<void> | void; onRefreshFiles?: () => Promise<void> | void; onOpenFile?: (file: WorkspaceFileSummary) => Promise<void> | void }) {
  ...
  useEffect(() => {
    ...
    const tools=drawingControls(instance,initial,{
      ...
      change:features=>{setImports(features);try {localStorage.setItem(storageKey,JSON.stringify(features));setStorageError('');}catch {setStorageError('未能保存本机几何，请先复制代码或保留当前窗口。');}for (const feature of features) { const owner = projectLayers.find(layer => layer.features.some(item => selectionKey(item) === selectionKey(feature))); const original = owner?.features.find(item => selectionKey(item) === selectionKey(feature)); if (owner && original && JSON.stringify(original.geometry) !== JSON.stringify(feature.geometry)) void onUpdateFeature?.(feature); }},
  ...
  },[onUpdateFeature, projectLayers, workspaceKey]);
  useEffect(() => {
    const instance=map.current; if(!instance) return;
  ...
  useEffect(() => {
    ...
    for (const projectLayer of projectLayers) {
      ...
      const layer = L.geoJSON({ type: 'FeatureCollection', features: projectLayer.features } as GeoJSON.GeoJsonObject, {
        ...
        onEachFeature: (feature, item) => {
          const label = String(feature.properties?.name ?? feature.properties?.id ?? projectLayer.name);
          item.bindTooltip(`${projectLayer.name} · ${label}`);
          item.on('click', (event: L.LeafletMouseEvent) => {
            if (drawingRef.current || Date.now() < suppressClick.current) return;
