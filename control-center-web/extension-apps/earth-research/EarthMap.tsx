...
import { Check, MousePointer2, ListChecks, Magnet, MapPin, Pencil, Pentagon, Redo2, Save, Scan, Scissors, Spline, Trash2, Undo2, X } from 'lucide-react';
...
export function EarthMap({ run, onSelect, command, selection, workspaceKey, onActivity, onMapState, projectLayers = EMPTY_PROJECT_LAYERS, spatialSources = [], workspaceFiles = [], workspaceFilesIncomplete, workspaceFilesNotice, activeObjectLabel, onSaveLayer, onUpdateFeature, onUpdateFeatures, onCreateBundle, onExportLayer, onToggleLayer, onRemoveLayer, onConnectSource, onRefreshCatalog, onRefreshFiles, onOpenFile, localRuns,localResult,onLoadSourceLayer,onShowRun,onCompareRuns,onOpenLayerRevision }: EarthMapProps) {
  ...
  useEffect(() => {
    ...
    for (const projectLayer of projectLayers) {
      ...
      const layer = L.geoJSON({ type: 'FeatureCollection', features: projectLayer.features.filter(feature=>!pendingRemoved.includes(selectionKey(feature)) && !JSON.parse(editingSelectionKeys).includes(selectionKey(feature))) } as GeoJSON.GeoJsonObject, {
  ...
  return <div className="earth-map" data-drawing={drawing} onKeyDown={event=>{
    ...
    <div className="earth-map-tools">
      <fieldset className="earth-gis-toolbar" aria-label="地图绘制工具" disabled={geometrySaving}>
        <button type="button" className="earth-gis-toolbar__tool" aria-pressed={!drawing} title="选择与浏览地图 · Esc 取消当前编辑" onClick={()=>geometryTools.current?.cancel()}><MousePointer2 size={16} aria-hidden="true"/><span>选择</span></button>
        <button type="button" className="earth-gis-toolbar__tool" disabled={['edit','cut','remove'].includes(drawingKind ?? '')} aria-pressed={drawingKind === 'point'} onClick={() => geometryTools.current?.start('point')}><MapPin size={16} aria-hidden="true" /><span>点</span></button>
        <button type="button" className="earth-gis-toolbar__tool" disabled={['edit','cut','remove'].includes(drawingKind ?? '')} aria-pressed={drawingKind === 'polyline'} onClick={() => geometryTools.current?.start('polyline')}><Spline size={16} aria-hidden="true" /><span>线</span></button>
        <button type="button" className="earth-gis-toolbar__tool" disabled={['edit','cut','remove'].includes(drawingKind ?? '')} aria-pressed={drawingKind === 'polygon'} onClick={() => geometryTools.current?.start('polygon')}><Pentagon size={16} aria-hidden="true" /><span>面</span></button>
        <button type="button" className="earth-gis-toolbar__tool" disabled={['edit','cut','remove'].includes(drawingKind ?? '')} aria-pressed={drawingKind === 'rectangle'} onClick={() => geometryTools.current?.start('rectangle')}><Scan size={16} aria-hidden="true" /><span>矩形</span></button>
        ...
        {drawingKind === 'edit' || drawingKind === 'remove' || drawingKind === 'cut' ? <>
          ...
          <button type="button" className="earth-gis-toolbar__tool" disabled={drawingVertexCount===0} onClick={()=>geometryTools.current?.undoVertex()}><Undo2 size={16} aria-hidden="true"/><span>撤销顶点</span></button>
          <button type="button" className="earth-gis-toolbar__tool" onClick={() => geometryTools.current?.cancel()}><X size={16} aria-hidden="true" /><span>取消</span></button>
        </> : null}
      </fieldset>
      ...
      <span role="status">{geometrySaving ? '正在保存几何，完成后可继续编辑' : drawingKind === 'edit' ? '拖动顶点后点击“保存”更新几何' : drawingKind === 'cut' ? '绘制内部范围，完成后保存；取消可还原' : drawingKind === 'remove' ? '所选对象已暂存删除 · 保存后生效，撤销或取消可恢复' : drawingKind === 'polygon' ? `面：已添加 ${drawingVertexCount} 个点 · 继续点击添加，双击或“完成”结束` : drawingKind === 'polyline' ? `线：已添加 ${drawingVertexCount} 个点 · 继续点击添加，双击或“完成”结束` : drawingKind === 'point' ? '单击放置点 · Esc 取消' : drawingKind === 'rectangle' ? '拖动绘制矩形 · Esc 取消' : drawing ? '绘图中 · Esc 取消' : selectionNeedsProjectLayer ? readOnlySelectionHint : selection.length ? `已选 ${selection.length} 个 · 可编辑或删除所选` : multiSelect ? '点击选择，再次点击取消选择' : '单击选择 · Shift 多选 · 空白处取消选择'}</span>
    </div>
