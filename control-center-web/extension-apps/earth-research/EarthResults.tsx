...
export function EarthResults({ run }: { run: EarthRun | null }) {
  ...
  return <section className="earth-results" aria-label="运行控制台">
    ...
    {run.tasks?.length ? <section className="earth-run-assets"><h3>云端任务</h3>{run.tasks.map((task, index) => <p key={`${task.id ?? 'unknown'}-${index}`}><code>{task.id ?? '无 task ID'}</code> · {task.kind}/{task.destination} · {task.status}</p>)}</section> : null}
    {run.artifacts?.length ? <section className="earth-run-assets"><h3>本地成果</h3>{run.artifacts.map((artifact, index) => <p key={`${artifact.path}-${index}`}><code>{artifact.kind}</code> · {artifact.path} · {artifact.bytes ?? 0} bytes</p>)}</section> : null}
