...
export function selectGeometries(state:GisState,ids:string[],mode:'replace'|'add'|'remove'|'toggle'='replace'):GisState {const current=new Set(state.selectedIds);if(mode==='replace'){return {...state,selectedIds:[...new Set(ids)]}};for(const id of ids){if(mode==='remove'||(mode==='toggle'&&current.has(id)))current.delete(id);else current.add(id)}return {...state,selectedIds:[...current]};}
